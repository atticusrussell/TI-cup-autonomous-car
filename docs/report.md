# 2 Fast 2 Delirious: TI Cup Autonomous Car
...
### Previous Efforts
Prior to the TI Cup, students competed in a similar competition called the NXP Cup. The objective of the two races are the same in which students program an autonomous car to compete for the fastest time around an unknown track. The main difference is that an ARM FRDM-K64F microcontroller was used in the NXP Cup [^2^], while a TI MSP432 microcontroller, which also utilized an ARM processor, was used in the TI Cup.

In previous competitions similar to the TI Cup, students competing in the TI Cup utilized an algorithm that applied a binary filter on the line-scan data and used the average of those values to establish a threshold that determined the course of the car. The car stopped when the amount of data points went under the threshold and used the average of the data points to steer the car along the track when the points were above the threshold [^3^].

Another method a previous team used is the bang-bang strategy in which the car is programmed to adjust its speed automatically depending on the consistency of its navigation. The implementation of this method allows the car to control its speed when it determines a pattern in the types of turns or directions it is going in, however, it is not a sophisticated control method and is not suitable for higher speeds [^4^].

### PID Control
PID is a control algorithm that uses feedback to control systems. The algorithm calculates the error between the desired output and the actual output of the system and applies corrective actions to bring the output closer to the desired value. The three terms in the algorithm, Proportional, Integral, and Derivative, represent different aspects of the error and are used to calculate the corrective action. Fig. [^5^] displays a block diagram of a PID controller in a closed feedback loop.
...
[^1^]: TI Cup Car Bill of Materials [\[source\]](BeatoBom)
[^2^]: NXP Cup [\[source\]](NXPPaper)
[^3^]: Previous algorithm using binary filter [\[source\]](HarrisonPaper)
[^4^]: Previous bang-bang strategy [\[source\]](DantePaper)
[^5^]: PID Controller Block Diagram [\[source\]](pidDiagram)
[^6^]: Line-Scan Camera Timing [\[source\]](ourLab6)
[^7^]: Line-Scan Camera Timing Diagram [\[source\]](cameraTiming)
[^8^]: Wiring Diagram for Line-Scan Camera [\[source\]](labManual)
...
When not being used for mode selection or countdown, the color of LED2 was set based on the magnitude of the visual center of mass value to provide useful insight about the car's perception during development and tweaking the camera position.
...
## References
[^1]: A. Russell, Photo of TI Car. 7 Dec. 2022
2. H. Barnes, B. Wrighter, "Designing Autonomous Driving in the 2021 Texas Instruments Cup", 07-Dec-2022.
3. L.J. Beato, private communication. 6 Dec. 2022
4. Y. Yang, "Introduction to PID Control Loops". Erdosmiller, 03-Mar-2022. [Online]. Available: [https://info.erdosmiller.com/blog/introduction-to-pid-control-loops](https://info.erdosmiller.com/blog/introduction-to-pid-control-loops)
5. Line Scan Camera Use. (n.d.). NXP Community. [Online]. Available: [https://community.nxp.com/t5/University-Programs-Knowledge/Line-Scan-Camera-Use/ta-p/1105313\#toc-hId--1874155339](https://community.nxp.com/t5/University-Programs-Knowledge/Line-Scan-Camera-Use/ta-p/1105313\#toc-hId--1874155339)
6. L. Beato. "CMPE-460 Interface & Digital Electronics Laboratory Manual". Department of Computer Engineering, Rochester Institute of Technology
7. A. Russell, E. Coles, "CMPE-460 Laboratory Exercise 5, MSP432 Timers, Interrupts, and Analog-to-Digital Converter", 9 Oct. 2022.
8. A. Russell, E. Coles, "CMPE-460 Laboratory Exercise 6, Motor Control", 19 Oct. 2022
9. H. Barnes, Photo of NXP Cup Track. 3 Dec. 2022
10. D. Sivo, A. Tarmu, "Demonstration of Small Autonomous Vehicles Through the Texas Instruments Cup", 07 Dec 2022.
11. L. Burnette and K. Carlile, "The NXP Cup; Building an Autonomous Racecar," 5 May 2021.
